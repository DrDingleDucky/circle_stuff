...
Circle::Circle(float gravity, float mass, float damping, float friction,
               float bounce, float radius, sf::Color color,
...
void Circle::Update(sf::RenderWindow &window, float deltaTime,
...
void Circle::MouseCollision(sf::RenderWindow &window) {
  ...
  float distance = sqrt(pow(sf::Mouse().getPosition(window).x - circleX, 2) +
                        pow(sf::Mouse().getPosition(window).y - circleY, 2));
...
void Circle::Collision(sf::RenderWindow &window) {
  if (circle.getPosition().x > window.getSize().x) {
    circle.setPosition(
        sf::Vector2f(0.0f - circle.getRadius() * 2, circle.getPosition().y));
  } else if (circle.getPosition().x + circle.getRadius() * 2 < 0.0f) {
    circle.setPosition(
        sf::Vector2f(window.getSize().x, circle.getPosition().y));
  }
}

void Circle::Movement(sf::RenderWindow &window, float deltaTime,
                      sf::RectangleShape floor) {
  if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
    if (mouseTouched) {
      ...
      float distance =
          sqrt(pow(sf::Mouse().getPosition(window).x - circleX, 2) +
               pow(sf::Mouse().getPosition(window).y - circleY, 2));

      line.setSize(sf::Vector2f(-distance, 4));
      line.setPosition(sf::Vector2f(sf::Mouse::getPosition(window).x - 2.0f,
                                    sf::Mouse::getPosition(window).y - 2.0f));
